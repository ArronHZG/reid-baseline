...
# Main interface
cpdef evaluate_cy(indices, q_feats, g_feats, q_pids, g_pids, q_camids, g_camids, max_rank):
    ...
    return eval_market1501_cy(indices, q_feats, g_feats, q_pids, g_pids, q_camids, g_camids, max_rank)


cpdef eval_market1501_cy(long[:,:] indices, float[:,:] q_feats, float[:,:] g_feats, long[:] q_pids, long[:]g_pids,
    ...
    return np.asarray(avg_cmc).astype(np.float32), np.mean(all_AP[:valid_index])
